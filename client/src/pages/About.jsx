...
const defaultContent = {
  ...
  foundersSubtitle:
    "Visionaries reviving Bharateeya wisdom with a practical, future-ready model of learning.",
  foundersDescription:
  ...
  foundersNote:
    "Discover their full journey, mission, and educational philosophy on the dedicated founders page.",
  ...
  whyCards: [
    ...
    },
    {
      id: "w5",
      icon: "Building2",
      title: "Luxury Living in Forest Setting",
      text: "Live and learn in harmony with nature while enjoying modern conveniences, just moments from the city.",
    },
    {
      id: "w6",
      icon: "Orbit",
      title: "Suitable for NRIs returning to Bharat",
      text: "Ideal for NRIs returning to Bharat, offering a seamless transition with a culturally immersive and academically enriching environment.",
    },
    {
      id: "w7",
      icon: "Leaf",
      title: "Organic Living",
      text: "Enjoy organic milk from desi cows, freshly grown organic vegetables, and a thriving food forest, embracing a healthy lifestyle in harmony with nature.",
    },
    {
      id: "w8",
      icon: "BookOpen",
      title: "Samskrutham Expert",
      text: "Children can attain profound proficiency in Samskrutham, paving the way to becoming scholars and conducting in-depth research on ancient scriptures.",
    },
  ],
};
...
const normalize = (content = {}) => ({
  ...
  ...content,
  aboutHighlights:
    Array.isArray(content.aboutHighlights) && content.aboutHighlights.length
      ? content.aboutHighlights
      : defaultContent.aboutHighlights,
  whyCards:
    Array.isArray(content.whyCards) && content.whyCards.length
      ? content.whyCards
      : defaultContent.whyCards,
});
...
export default function About() {
  ...
  const applyEditor = () => {
    if (!editor) return;
    setDraft((prev) => {
      if (editor.type === "intro")
        return {
          ...prev,
          introLabel: editor.introLabel,
          introTitle: editor.introTitle,
        };
      ...
      if (editor.type === "whyTitle")
        return { ...prev, whyTitle: editor.whyTitle };
            ...
            {isAdmin ? (
              <button
                ...
                onClick={() =>
                  setEditor({
                    type: "intro",
                    introLabel: display.introLabel,
                    introTitle: display.introTitle,
                  })
                }
                className="mb-2 rounded-md bg-white/90 p-1 text-accent shadow dark:bg-neutral-800 dark:text-emerald-200"
              >
          ...
          <div className="grid gap-5 md:gap-6">
                    ...
                    <div className="mb-3 flex justify-end gap-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setDraft((p) => ({
                            ...p,
                            aboutHighlights: p.aboutHighlights.filter(
                              (_, idx) => idx !== i,
                            ),
                          }))
                        }
            ...
            {isAdmin ? (
              <button
                ...
                className="rounded-xl border border-dashed border-neutral-300 px-4 py-3 text-sm text-neutral-700 dark:border-neutral-700 dark:text-neutral-200"
              >
                + Add Highlight Card
              </button>
            ) : null}
          ...
          {isAdmin ? (
            <button
              ...
              onClick={() =>
                setEditor({
                  type: "founders",
                  foundersTitle: display.foundersTitle,
                  foundersSubtitle: display.foundersSubtitle,
                  founder1Name: display.founder1Name,
                  founder2Name: display.founder2Name,
                  foundersDescription: display.foundersDescription,
                  foundersNote: display.foundersNote,
                  foundersButtonLabel: display.foundersButtonLabel,
                  foundersButtonLink: display.foundersButtonLink,
                })
              }
              className="mb-2 rounded-md bg-white/90 p-1 text-accent shadow dark:bg-neutral-800 dark:text-emerald-200"
            >
                  ...
                  {isAdmin ? (
                    <div className="absolute inset-x-2 top-2 z-10 flex justify-end">
                      <ImageUploader
                        ...
                        buttonText="Change"
                        onUploaded={(asset) =>
                          setDraft((p) => ({ ...p, founderImage1: asset.url }))
                        }
                      />
                    </div>
                  ) : null}
                  ...
                  {isAdmin ? (
                    <div className="absolute inset-x-2 top-2 z-10 flex justify-end">
                      <ImageUploader
                        ...
                        buttonText="Change"
                        onUploaded={(asset) =>
                          setDraft((p) => ({ ...p, founderImage2: asset.url }))
                        }
                      />
                    </div>
                  ) : null}
              ...
              <div className="flex max-w-xl flex-col gap-6 lg:max-w-md">
                ...
                <div className="rounded-xl border border-neutral-200 bg-secondary/50 p-4 dark:border-neutral-700 dark:bg-neutral-800">
                  <p className="text-sm leading-relaxed text-prose-muted">
                    {display.foundersNote}
                  </p>
                </div>
                <Button
                  to={display.foundersButtonLink || "/founders"}
        ...
        <Container>
          {isAdmin ? (
            <button
              type="button"
              onClick={() =>
                setEditor({ type: "whyTitle", whyTitle: display.whyTitle })
              }
              className="mb-2 rounded-md bg-white/90 p-1 text-accent shadow dark:bg-neutral-800 dark:text-emerald-200"
            >
              <Pencil className="h-4 w-4" />
                    ...
                    <div className="mb-3 flex justify-end gap-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setDraft((p) => ({
                            ...p,
                            whyCards: p.whyCards.filter((_, idx) => idx !== i),
                          }))
                        }
                        className="rounded-md bg-rose-100 p-1 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300"
                      >
          ...
          <div className="w-full max-w-2xl rounded-2xl border border-neutral-200 bg-white p-5 dark:border-neutral-700 dark:bg-neutral-900">
            ...
            {editor.type === "intro" ? (
              <div className="space-y-3">
                <input
                  ...
                  value={editor.introLabel}
                  onChange={(e) =>
                    setEditor((p) => ({ ...p, introLabel: e.target.value }))
                  }
                />
                <input
                  className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                  value={editor.introTitle}
                  onChange={(e) =>
                    setEditor((p) => ({ ...p, introTitle: e.target.value }))
                  }
                />
              </div>
            ) : null}
            {editor.type === "founders" ? (
              <div className="space-y-2">
                <input
                  className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                  value={editor.foundersTitle}
                  onChange={(e) =>
                    setEditor((p) => ({ ...p, foundersTitle: e.target.value }))
                  }
                />
                <input
                  className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                  value={editor.foundersSubtitle}
                  onChange={(e) =>
                    setEditor((p) => ({
                      ...p,
                      foundersSubtitle: e.target.value,
                    }))
                  }
                />
                <input
                  className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                  value={editor.founder1Name}
                  onChange={(e) =>
                    setEditor((p) => ({ ...p, founder1Name: e.target.value }))
                  }
                />
                <input
                  className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                  value={editor.founder2Name}
                  onChange={(e) =>
                    setEditor((p) => ({ ...p, founder2Name: e.target.value }))
                  }
                />
                <textarea
                  ...
                  value={editor.foundersDescription}
                  onChange={(e) =>
                    setEditor((p) => ({
                      ...p,
                      foundersDescription: e.target.value,
                    }))
                  }
                />
                <textarea
                  className="h-16 w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                  value={editor.foundersNote}
                  onChange={(e) =>
                    setEditor((p) => ({ ...p, foundersNote: e.target.value }))
                  }
                />
                <input
                  className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                  value={editor.foundersButtonLabel}
                  onChange={(e) =>
                    setEditor((p) => ({
                      ...p,
                      foundersButtonLabel: e.target.value,
                    }))
                  }
                />
                <input
                  className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                  value={editor.foundersButtonLink}
                  onChange={(e) =>
                    setEditor((p) => ({
                      ...p,
                      foundersButtonLink: e.target.value,
                    }))
                  }
                />
              </div>
            ...
            {editor.type === "highlight" || editor.type === "whyCard" ? (
              <div className="space-y-3">
                <select
                  ...
                  value={editor.icon}
                  onChange={(e) =>
                    setEditor((p) => ({ ...p, icon: e.target.value }))
                  }
                >
                  {iconNames.map((name) => (
                ...
                <textarea
                  ...
                  value={editor.body ?? editor.text}
                  onChange={(e) =>
                    setEditor((p) => ({
                      ...p,
                      [editor.type === "highlight" ? "body" : "text"]:
                        e.target.value,
                    }))
                  }
                />
            ...
            {editor.type === "whyTitle" ? (
              <input
                className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100"
                value={editor.whyTitle}
                onChange={(e) =>
                  setEditor((p) => ({ ...p, whyTitle: e.target.value }))
                }
              />
            ) : null}
            <div className="mt-4 flex gap-2">
              ...
              <button
                type="button"
                onClick={() => setEditor(null)}
                className="rounded-lg bg-neutral-200 px-4 py-2 text-sm text-neutral-800 dark:bg-neutral-700 dark:text-neutral-100"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
          <div className="pointer-events-auto w-full max-w-sm rounded-2xl border border-neutral-200 bg-white p-5 shadow-xl dark:border-neutral-700 dark:bg-neutral-900">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => setDraft(normalized)}
                className="w-full rounded-lg bg-neutral-200 px-4 py-2 text-sm text-neutral-800 dark:bg-neutral-700 dark:text-neutral-100 sm:w-auto"
              >
                Reset
              </button>
            </div>

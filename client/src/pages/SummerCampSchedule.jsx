  ...
  tabs: {
      ...
      contentBlocks: [
        {
          ...
          items: [
            ...
            "6:30 AM: Personal hygiene activities with guidance on how to maintain a healthy lifestyle, including drinking warm water, attending nature's call, and bathing.",
    ...
    activity: {
      ...
      contentBlocks: [
        {
          type: "paragraph",
          text: "Engage in group activities with peers. Each session lasts 45-60 minutes and takes place in a combinations of outdoors and air-conditioned indoor halls.",
        },
        {
          type: "subheading",
      ...
      contentBlocks: [
        {
          ...
          items: [
            ...
            "7:30 PM - 8:00 PM: Dinner break, offering a delicious vegetarian meal with North and South Indian cuisine.",
            "8:00 PM - 8:30 PM: Prayer and Sayanakaala Mantras.",
            "8:30 PM - 8:45 PM: Evening walk (Bramanam) accompanied by discussions on Chanakya Neeti, Bartruhari Neethi, and Slokas.",
      ...
      contentBlocks: [
        {
          ...
          items: [
            ...
            "11:00 AM: Snacks",
            "1:00 PM: Nutritious vegetarian lunch featuring both North / South Indian dishes.",
            "4:30 PM: Snacks",
            "7:30 PM: Dinner serving a delectable vegetarian meal with North / South Indian cuisine.",
          ],
        },
      ],
    },
...
export default function SummerCampSchedule() {
  const location = useLocation();
  const isAdmin = location.pathname === "/admin/summer-camp/schedule";
  const [activeTab, setActiveTab] = useState("morning");
  const [introEditor, setIntroEditor] = useState(null);
  const [tabEditor, setTabEditor] = useState(null);
  const cms = useLiveContent("summer-camp-schedule", defaultScheduleContent);
  const [draft, setDraft] = useState(cms);

  useEffect(() => setDraft(cms), [cms]);

  const saveScheduleContent = async (nextContent) => {
    await adminRequest("/api/admin/content/summer-camp-schedule", {
      method: "PUT",
  ...
  const detailsPath = isAdmin ? "/admin/summer-camp" : "/summer-camp";
  ...
  return (
    <PageFade>
      <section className="border-b border-neutral-200 bg-secondary/35 py-10 dark:border-neutral-700 dark:bg-neutral-950/70 md:py-14">
        <Container>
          <h1 className="heading-page">Summer Camp Schedule</h1>
      ...
      <Container className="py-8 md:py-12">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            {renderContentBlocks(resolvedIntroBlocks, "scs-intro", "mt-0")}
          </div>
          {isAdmin ? (
            <button
              type="button"

...
export default function SummerCampDetail() {
  ...
  const backPath = isAdmin ? '/admin/summer-camp' : '/summer-camp';
  ...
  const saveGallery = async (nextGallery) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Summer camp details updated.' });
      return true;
    } catch (error) {
    ...
    return (
      <PageFade>
        <Container className="py-12 md:py-16">
          ...
          <p className="mt-3 text-prose">The selected summer camp folder does not exist.</p>
  ...
  return (
    <PageFade>
      <Container className="py-12 md:py-16">
        ...
        <p className="mt-2 text-sm text-prose-muted">Summer: {galleryYear || '-'}</p>
        ...
        <section className="mt-10">
          <div className="mb-3 flex items-center justify-between gap-3">
            <h2 className="heading-section">About This Summer Camp</h2>
            {isAdmin ? (
              <button
                type="button"
      ...
      {metaEditor ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/40 p-4">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-neutral-200 bg-white p-5 dark:border-neutral-700 dark:bg-neutral-900">
            <h3 className="text-lg font-semibold text-accent dark:text-emerald-200">Edit Summer Camp Details</h3>
